...
export default function RoboMasterTeamHome() {
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white selection:bg-white/10 selection:text-white">
      <NeonBackdrop />
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-black/60 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6">
          <a href="#home" className="group inline-flex items-center gap-2">
            ...
            <span className="text-lg font-semibold tracking-wide">RoboMaster 战队</span>
          ...
          <div className="flex items-center gap-3">
            <GradientButton as="a" href="#join" className="hidden md:inline-flex">
              立即加入
            </GradientButton>
          </div>
        </div>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 md:grid-cols-2 md:gap-12 md:px-6 lg:py-24">
          <div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <GradientButton as="a" href="#groups">
                探索小组
              </GradientButton>
              <GhostButton as="a" href="#links" className="gap-1">
                <LinkIcon className="h-4 w-4" /> 官方链接
              </GhostButton>
            </div>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4" id="links-quick">
              ...
              <SocialLink href="#" label="Bilibili">
                <Youtube className="h-5 w-5" />
              </SocialLink>
              <SocialLink href="#" label="微博">
                <Globe className="h-5 w-5" />
              </SocialLink>
              <SocialLink href="#" label="Instagram">
                <Instagram className="h-5 w-5" />
              </SocialLink>
              <SocialLink href="mailto:team@example.com" label="Email">
      ...
      <section id="groups" className="relative">
        <SectionTitle
          title="分工协作 · 五大核心小组"
          subtitle="点击任意卡片打开子主页，查看职责/技能栈/在招方向。"
        />
      ...
      <section id="projects" className="relative">
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-5 px-4 pb-14 md:grid-cols-2 md:px-6 lg:grid-cols-3">
          {[
            {
              title: '智能自瞄系统',
              desc: '融合目标检测/位姿估计与预测补偿，上位机到嵌入式端到端管线。',
              badge: '视觉',
            },
            {
              title: '高功率驱动模组',
              desc: '高效电机驱动与电源管理，一体化布局提升可靠性与维护性。',
              badge: '硬件',
            },
            {
              title: '多车协同通信',
              desc: '基于 CAN/以太网的车队通信协议，任务分配与容错机制。',
              badge: '电控',
            },
          ].map((p, i) => (
            <ProjectCard key={i} {...p} />
      ...
      <section id="links" className="relative">
        <SectionTitle title="社交媒体 & GitHub" subtitle="替换为你们的真实链接" />
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-4 px-4 pb-14 md:grid-cols-2 md:px-6 lg:grid-cols-3">
          <LinkRow icon={Github} label="GitHub 组织" href="#" note="开源代码与文档" />
          <LinkRow icon={Youtube} label="Bilibili" href="#" note="赛事视频/周更日志" />
          <LinkRow icon={Globe} label="官网/博客" href="#" note="技术文章与团队动态" />
          <LinkRow icon={Instagram} label="Instagram" href="#" note="日常花絮与幕后" />
          <LinkRow icon={Mail} label="团队邮箱" href="mailto:team@example.com" note="对外合作/招新咨询" />
        </div>
      </section>
            ...
            <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
              <div>
                ...
                <p className="mt-2 max-w-xl text-white/70">
                  填写报名表，我们会在 72 小时内邮件回复并安排群面/试工坊。
                </p>
                <div className="mt-3 inline-flex items-center gap-3 text-sm text-white/60">
                  <MapPin className="h-4 w-4" /> 校园创客空间 · 周一/三/五 晚
                </div>
              </div>
              <div className="flex shrink-0 items-center gap-3">
                <GradientButton as="a" href="#" className="">
                  填写报名表
                </GradientButton>
                <GhostButton onClick={() => copyToClipboard('team@example.com')} aria-label="复制邮箱">
...
function GradientButton({ as: As = 'button', className = '', children, ...props }: GradientButtonProps) {
  return (
    <As
      className={`inline-flex items-center justify-center rounded-2xl border border-white/10 bg-gradient-to-r from-fuchsia-500 via-cyan-400 to-emerald-400 p-[1.5px] text-sm font-semibold text-white shadow-[0_0_24px_rgba(77,151,255,0.25)] ${className}`}
      {...props}
    >
...
function GhostButton({ as: As = 'button', className = '', children, ...props }: GradientButtonProps) {
  return (
    <As
      className={`inline-flex items-center justify-center rounded-2xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-medium text-white/90 backdrop-blur transition hover:bg-white/10 ${className}`}
      {...props}
    >
      {children}
    </As>
...
function SocialLink({ href, label, children }: SocialLinkProps) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="card-hover inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/80 hover:text-white"
      aria-label={label}
      title={label}
    >
      {children}
...
function HeroShowcase() {
  return (
    <div className="relative">
      <div className="relative mx-auto aspect-[4/3] w-full max-w-xl overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-white/10 to-transparent p-4">
...
function Block({ title, icon: Ico, colorClass, lines = 4 }: BlockProps) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/60 p-3 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.05)]">
      ...
      <div className="relative flex items-center gap-2">
        <div className={`flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-r ${colorClass} p-[1.5px]`}>
          <div className="h-full w-full rounded-[10px] bg-black p-1">
            <Ico className="h-full w-full" />
          </div>
        </div>
...
function GroupCard({ meta, onOpen }: { meta: GroupMeta; onOpen: () => void }) {
  ...
  return (
    <button
      onClick={onOpen}
      className="card-hover group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-5 text-left"
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className={`flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-r ${meta.gradient} p-[2px]`}>
            <div className="h-full w-full rounded-[14px] bg-black p-1">
...
function GroupModal({ meta, onClose }: { meta: GroupMeta; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4">
      ...
      <div className="relative z-10 w-full max-w-3xl overflow-hidden rounded-3xl border border-white/10 bg-black">
        ...
        <div className="relative p-6 md:p-8">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <InfoPanel title="技能栈">
              <div className="flex flex-wrap gap-2">
                {meta.stack.map((s, i) => (
                  <span key={i} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/80">
                    {s}
                  </span>
                ))}
              </div>
            ...
            <InfoPanel title="在招方向">
              <div className="flex flex-wrap gap-2">
                {meta.recruiting.map((s, i) => (
                  <span key={i} className="rounded-full border border-white/10 bg-gradient-to-r from-white/10 to-white/5 px-3 py-1 text-xs text-white">
                    {s}
                  </span>
                ))}
              </div>
          ...
          <div className="mt-6">
            <h4 className="mb-2 text-sm font-semibold text-white/80">资源链接</h4>
            <div className="flex flex-wrap gap-3">
              {meta.links.map((l, i) => (
                <a
                  key={i}
                  href={l.href}
                  className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/80 hover:text-white"
                >
...
function ProjectCard({ title, desc, badge }: { title: string; desc: string; badge: string }) {
  return (
    <div className="card-hover relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full border border-white/10 bg-black/50 px-2 py-1 text-xs text-white/70">{badge}</span>
      </div>
      <p className="mt-2 text-sm text-white/70">{desc}</p>
      <div className="mt-4 inline-flex items-center gap-1 text-sm text-white/80">
        <a href="#" className="rounded-lg border border-white/10 bg-white/5 px-2 py-1">
          文档
        </a>
        <a href="#" className="rounded-lg border border-white/10 bg-white/5 px-2 py-1">
          仓库
...
function LinkRow({ icon: Icon, label, href, note }: LinkRowProps) {
  return (
    <a
      href={href}
      target="_blank"
        ...
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div>
            <div className="inline-flex items-center gap-2 text-white/90">
              <div className="h-6 w-6 rounded-lg bg-gradient-to-tr from-fuchsia-500 via-cyan-400 to-emerald-400 p-[1.5px]">
                <div className="h-full w-full rounded-[10px] bg-black" />
              </div>
              <span className="font-semibold">RoboMaster 战队</span>
            </div>
            <p className="mt-2 text-sm text-white/60">© {new Date().getFullYear()} All rights reserved.</p>
          </div>
